...
def _migrate_task_messages_v2_to_v3(sync_connection) -> None:
    ...
    for target_column in Task.__table__.columns:
        ...
        elif column_name in legacy_task.c:
...
def _migrate_task_checkpoint_v9_to_v10(sync_connection) -> None:
    existing_columns = {
        column["name"]
        for column in inspect(sync_connection).get_columns(Task.__tablename__)
    }
    if "checkpoint" not in existing_columns:
        sync_connection.execute(
            text(f'ALTER TABLE "{Task.__tablename__}" ADD COLUMN checkpoint JSON')
        )
    task_table = Table(
        Task.__tablename__,
        MetaData(),
...
def _migrate_task_summary_v10_to_v11(sync_connection) -> None:
    existing_columns = {
        column["name"]
        for column in inspect(sync_connection).get_columns(Task.__tablename__)
    }
    if "primary_operation" not in existing_columns:
        sync_connection.execute(
            text(
                f'ALTER TABLE "{Task.__tablename__}" '
                "ADD COLUMN primary_operation VARCHAR NOT NULL DEFAULT ''"
            )
        )
    if "summary_artifacts" not in existing_columns:
        sync_connection.execute(
            text(
                f'ALTER TABLE "{Task.__tablename__}" '
                "ADD COLUMN summary_artifacts JSON NOT NULL DEFAULT '[]'"
            )
        )
...
_TASK_MIGRATIONS = {
    ...
    9: _migrate_task_checkpoint_v9_to_v10,
    10: _migrate_task_summary_v10_to_v11,
}
