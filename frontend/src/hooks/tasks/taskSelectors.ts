...
type TranslateTaskRequestParams = {
  mode?: string;
  context_path?: string;
};
...
export const getTranslationTaskMode = (task: Task): TranslatorMode | null => {
  const rawMode = (task.request_params as TranslateTaskRequestParams | undefined)?.mode;
...
export const findActiveTranslationTask = (
  ...
  tasks.find((task) => {
    ...
    const params = task.request_params as TranslateTaskRequestParams | undefined;
    if (sourceFilePath && params?.context_path) {
      return params.context_path === sourceFilePath;
    }

    return !sourceFilePath;
  });

const hasTranscribeStep = (task: Task): boolean => {
...
const getTranscribeAudioPath = (task: Task): string | null => {
  if (task.type === "transcribe") {
    const params = task.request_params as Record<string, unknown> | undefined;
    return typeof params?.audio_path === "string" ? params.audio_path : null;
  }

  const steps = (task.request_params as PipelineRequest | undefined)?.steps;
  if (!Array.isArray(steps)) return null;

  const transcribeStep = steps.find((step) => step.step_name === "transcribe");
  const params = transcribeStep?.params as Record<string, unknown> | undefined;
  return typeof params?.audio_path === "string" ? params.audio_path : null;
};
...
export const findActiveTranscribeTask = (
  ...
  tasks.find((task) => {
    ...
    return getTranscribeAudioPath(task) === filePath;
  });
...
export const mapTaskToTranscribeResult = (
  ...
  return {
    ...
    video_path: filePath ?? undefined,
    audio_path: filePath ?? undefined,
  };
};
