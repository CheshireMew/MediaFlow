import type { CleanRequest, OCRExtractRequest } from "../../../types/api";
...
type CleanupMethod = NonNullable<CleanRequest["method"]>;

function isOcrEngine(value: unknown): value is OcrEngine {
  return value === "rapid" || value === "paddle";
}

function isCleanupMethod(value: unknown): value is CleanupMethod {
  return value === "telea" || value === "navier";
}
...
async function submitCleanRetry(task: Task): Promise<RetrySubmission | null> {
  ...
  const rawMethod = readOptionalString(params.method);
  if (rawMethod !== undefined && !isCleanupMethod(rawMethod)) {
    return null;
  }
  const method = rawMethod as CleanupMethod | undefined;
